use fenris::assembly::local::{assemble_element_mass_matrix, assemble_element_elliptic_matrix};
use fenris::element::*;
use nalgebra::{DMatrix, U3, U2, DVector, DVectorSlice, DMatrixSliceMut, OMatrix, MatrixSliceMut};
use fenris::quadrature::{CanonicalMassQuadrature, CanonicalStiffnessQuadrature, Quadrature, QuadraturePair3d, QuadraturePair2d};
use fenris::quadrature;
use matrixcompare::{assert_matrix_eq, compare_matrices};
use fenris::nalgebra::{RealField, DefaultAllocator, Dynamic};
...
fn assemble_mass_for_element<T, Element>(element: &Element, quadrature: impl Quadrature<T, Element::ReferenceDim>)
-> DMatrix<T>
where
    T: RealField,
...
fn assemble_stiffness_for_element<T, Element>(element: &Element, quadrature: impl Quadrature<T, Element::ReferenceDim>)
                                         -> DMatrix<T>
where
    T: RealField,
    Element: VolumetricFiniteElement<T>,
    ...
    ($test_name:ident, $element:ident, $reference_quadrature:expr, $quadrature_iter:expr,
        ...
        $assembly_fn:ident) => {
            ...
            fn $test_name() {
                ...
                assert_eq!(canonical_quadrature.weights().len(), minimal_exact_quadrature.weights().len());
            }
        }
}
...
fn tet_quadrature_iter() -> impl Iterator<Item=QuadraturePair3d<f64>> {
    (0 ..= 10)
        .map(|i| quadrature::total_order::tetrahedron(i).unwrap())
}
...
fn hex_quadrature_iter() -> impl Iterator<Item=QuadraturePair3d<f64>> {
    (1 ..= 8)
        .map(|i| quadrature::tensor::hexahedron_gauss(i))
}
...
fn quad_quadrature_iter() -> impl Iterator<Item=QuadraturePair2d<f64>> {
    (1 ..= 8)
        .map(|i| quadrature::tensor::quadrilateral_gauss(i))
}
...
fn tri_quadrature_iter() -> impl Iterator<Item=QuadraturePair2d<f64>> {
    (0 ..= 10)
        .map(|i| quadrature::total_order::triangle(i).unwrap())
}

// Triangle elements
test_canonical_mass_assembly_is_exact_and_minimal!(
    Tri3d2Element,
    tri_reference_quadrature(),
    tri_quadrature_iter());
test_canonical_mass_assembly_is_exact_and_minimal!(
    Tri6d2Element,
    tri_reference_quadrature(),
